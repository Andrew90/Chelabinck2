...
bool ZonesViewer::CursorLabel::Draw(TMouseMove &l, VGraphics &g)
{
	...
	{
		offset = leftOffs;
		if(status == PrimaryData::Nominal || status == PrimaryData::DefectMax || status == PrimaryData::DefectMin)
		{
			sprintf(buf, "<0xff>Смещение <0xff0000>%d   <0xff>толщина <0xff0000>%0.2f %s   <ff>Номинальная толщина %.2f  Отклонение минус <ff0000>%.2f%%  <ff>плюс <ff0000>%.2f%%"
