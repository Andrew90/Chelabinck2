...
unsigned DetailedWindow::operator()(TCreate &m)
{
	...
	char buf[128];
	sprintf(buf,  "<ff>Номинальная толщина %.2f  <ff>Отклонение минус <ff0000>%.2f%%  <ff>плюс <ff0000>%.2f%% <ff>разностенность <ff0000>%.2f%%"
...
void DetailedWindow::Open()
{
	...
	if(NULL == x.hWnd)
	{
		WindowTemplate(&x, L"DetailedWindow", L"Просмотр датчиков", NULL
			, Singleton<ViewerWindowTable>::Instance().items.get<OffsetWindowX>().value
			, Singleton<ViewerWindowTable>::Instance().items.get<OffsetWindowY>().value
			, Singleton<ViewerWindowTable>::Instance().items.get<WindowWidth>().value
			, Singleton<ViewerWindowTable>::Instance().items.get<WindowHeight>().value
			);
	}
	x.Update();
}
