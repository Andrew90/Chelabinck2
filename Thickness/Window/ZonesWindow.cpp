...
{
	int count = int(currentAutomat->primaryData.offsetOfTime[0] * frequencyFrames * 0.001);
	offset = 0;
	if(zone_ > 0) 
	{
		int t = int(currentAutomat->primaryData.offsetOfTime[zone_] * frequencyFrames * 0.001);
		if(t > currentAutomat->primaryData.GetCurrentOffset()) t = currentAutomat->primaryData.GetCurrentOffset() - 2;
		offset = int(currentAutomat->primaryData.offsetOfTime[zone_ - 1] * frequencyFrames * 0.001);
		count = t - offset + 1;
	...
	if(0 != count)
	{
		...
		{
			zonesViewer.data.zones[i] = currentAutomat->primaryData.result[sensor][j];
			zonesViewer.data.status[i] = currentAutomat->primaryData.status[sensor][j];
		}
		wchar_t buf[128];
		wsprintf(buf, L"Просмотр первичного сигнала. Датчик %d Зона %d", 1 + sensor, 1 + zone);
		zonesViewer.data.countZones = count;
		zonesViewer.zonesOffset = offset;

		signalViewer.coefficientA = currentAutomat->compute.coefficientA[sensor];
		signalViewer.coefficientB = currentAutomat->compute.coefficientB[sensor];

		correlationViewer.coefficientA = currentAutomat->compute.coefficientA[sensor];
		correlationViewer.coefficientB = currentAutomat->compute.coefficientB[sensor];
		correlationViewer.minEnergy = currentAutomat->compute.minEnergy[sensor];

		acfViewer.coefficientA = currentAutomat->compute.coefficientA[sensor];
		acfViewer.coefficientB = currentAutomat->compute.coefficientB[sensor];
		acfViewer.peak = currentAutomat->compute.peak[sensor];
		acfViewer.minEnergy = currentAutomat->compute.minEnergy[sensor];

		if(0 == hWnd)
		{			
			WindowTemplate(this, L"ZonesWindow", L"");
		}
...
void ZonesWindow::MouseMoveHandler(unsigned offsetInZone)
{
	...
	char *s = currentAutomat->primaryData.SensorData(sensor, offs);
	...
	//{
		...
		ZeroMemory(data, sizeof(double) * currentAutomat->compute.acfBorderLeft);
		ZeroMemory(&data[currentAutomat->compute.acfBorderRight], sizeof(double) * (fft.bufferSize - currentAutomat->compute.acfBorderRight));
		//---------------------------------------------------------------------------------------
		fft.Direct(data);
		fft.Spectrum(data);		
		correlationViewer.thickness = currentAutomat->primaryData.result[sensor][offs];
		correlationViewer.status = currentAutomat->primaryData.status[sensor][offs];
	...
	//	{
			...
			correlationViewer.peak = currentAutomat->compute.peak[sensor] * data[0] + min;
...
//	}
	{
		...
		fft.Direct(data);
		fft.Spectrum(data);
		acfViewer.thickness = currentAutomat->primaryData.result[sensor][offs];
		double max = 0;
